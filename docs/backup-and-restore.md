...
Use the managed provider's encrypted automated backups where available, plus regular logical backups for portability. On free-tier managed databases, do not assume production-grade automated backups or point-in-time recovery are included. The examples below use standard PostgreSQL tools and make no provider assumptions.

## Backup

1. Choose a low-traffic period and record the application version, migration revision and UTC timestamp.
2. Run `pg_dump --format=custom --no-owner --no-acl --file=backups/distributoros-YYYYMMDDTHHMMSSZ.dump "$DATABASE_MIGRATION_URL"`.
3. Encrypt the artifact, store it outside the primary database environment, and record its checksum and retention expiry.
4. Verify with `pg_restore --list backups/distributoros-YYYYMMDDTHHMMSSZ.dump`.
...
2. Run `pg_restore --exit-on-error --clean --if-exists --no-owner --no-acl --dbname="$RESTORE_DATABASE_MIGRATION_URL" backups/distributoros-YYYYMMDDTHHMMSSZ.dump`.
